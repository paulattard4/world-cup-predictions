...
with st.expander("Google Sheet guide"):
    st.markdown(
        """
### Google Sheet tabs to edit
...
`match_predictions`  
One row per contestant per match.

`predicted_scorers`  
One row per predicted goal scorer. If a player is predicted to score twice, enter that player twice.

`results`  
Actual score and actual winner.

`actual_scorers`  
One row per actual goal. If a player scores twice, enter that player twice.

`group_predictions`  
One row per predicted qualified team.

`group_results`  
One row per actual qualified team.

`award_predictions`  
One row per contestant.

`award_results`  
One row only, once awards are known.
"""
    )
